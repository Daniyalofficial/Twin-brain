 ...
 *
 *  1. KNOWLEDGE CORE: ~100 curated world-knowledge topics (science, space,
 *     body, geography, history, money, tech, health, math, English, great
 *     lives, life skills) retrieved with the same BM25 machinery as the user's
 ...
 *
 *  3. BOOKSHELF: the five must-read success books (plus the billionaire
 *     bonus), taught from original full explanations — core idea, every major
 *     lesson with an action step, a seven-day plan, reading paths by ambition.
 */
...
/** The shelf answer: all five must-reads with one-line pitches + a reading path. */
export function bookshelf(query = '', { name = 'friend' } = {}) {
  const q = String(query || '').toLowerCase();
  let pathKey = 'default';
  if (/billionaire|rich|wealth|money|fortune/.test(q)) pathKey = 'billionaire';
  else if (/habit|discipline|routine|lazy|consisten/.test(q)) pathKey = 'habits';
  else if (/people|friend|talk|social|influence|communicat/.test(q)) pathKey = 'people';
  else if (/money|finance|invest|business|startup/.test(q)) pathKey = 'money';
  const path = (READING_PATHS[pathKey] || READING_PATHS.default)
    .map((id) => SUCCESS_BOOKS.find((b) => b.id === id))
    .filter(Boolean);

  const lines = [];
  lines.push(pathKey === 'billionaire'
    ? `${name === 'friend' ? '' : name + ' — '}here is THE shelf: the books successful people treat as required reading, in the order I would hand them to someone serious about wealth. I carry the FULL explanation of every one — ask me for any title by name.`
    : 'Here are the five books every ambitious person should read — I carry the full explanation of each one offline. Ask me for any title and I will teach it lesson by lesson:');
  path.forEach((book, i) => {
    lines.push('');
    lines.push(`${i + 1}. "${book.title}" — ${book.author} (${book.year})`);
    lines.push(`   ${book.oneLine}`);
  });
  if (pathKey === 'billionaire') {
    lines.push('');
    lines.push('The billionaire pattern across all six: master your mind (Hill), master money literacy (Kiyosaki), build something new instead of competing (Thiel), make it automatic (Clear), master people (Carnegie), and run your life on principles (Covey). No shortcut exists — but this order IS the shortcut everyone looks for.');
  }
  lines.push('');
...
/** Answer "how to become rich/billionaire" from the shelf: the combined path. */
export function successAnswer(query = '', { name = 'friend' } = {}) {
  const shelf = bookshelf(`${query} billionaire`, { name });
  const preamble = [
    `Straight answer, ${name === 'friend' ? 'friend' : name} — no hype, no get-rich schemes:`,
    '',
    'Billionaire-level wealth is never a salary outcome. It is built in four stages, and every self-made fortune I have studied follows them:',
    '1. MINDSET & DESIRE — a definite burning goal, written down, fed daily (Think and Grow Rich).',
    '2. MONEY LITERACY — know the difference between assets and liabilities; make money work for you (Rich Dad Poor Dad).',
    '3. OWNERSHIP & LEVERAGE — build or buy something that scales: a business, equity, code, media, property. Ownership, not hours, creates fortunes (Zero to One).',
    '4. SYSTEMS & PEOPLE — tiny daily disciplines compound (Atomic Habits), and no one scales alone (Carnegie + Covey).',
    '',
    'The honest timeline is a decade, not a month — but the decade passes anyway. The books below are the complete curriculum, and I carry every one of them in full:',
  ].join('\n');
