/**
 * MarketerTwin background service worker (Manifest V3) — extension-only:
 * no server, no localhost; every byte lives in the marktertwin IndexedDB.
 *
 * Responsibilities
...
import { handleMt, ensureMtAlarms, onMtAlarm } from './lib/mt.js';
...
async function updateBadge() {
  ...
  let text = '';
  let color = '#1d4ed8';
  let title = 'MarketerTwin';
  if (settings.globalPause || !settings.captureEnabled) {
    text = 'OFF';
    color = '#8a2b2b';
    title = 'MarketerTwin — capture is PAUSED (nothing is being recorded)';
  } else {
    const counts = await getCounts();
    if (settings.showBadgeCount && counts.captured > 0) {
      text = String(Math.min(counts.captured, 99));
      color = '#2f6f4f';
      title = `MarketerTwin — ${counts.captured} page(s) remembered today`;
    } else {
      title = 'MarketerTwin — capturing';
    }
  }
...
async function sendCapture(payload, tabState) {
  ...
  // MarketerTwin has no backend: the IndexedDB IS the database
  await store.cachePage(payload);
...
async function recordLinkOnly(tabState, reason) {
  ...
  await store.cachePage(payload);
  const result = { ok: true, local: true, link_only: true };
  await bumpCounts('captured');
  await snapshot();
  await updateBadge();
  return result;
...
async function heartbeat() {
  // MarketerTwin is standalone: no backend to ping. The badge is the only
  // surface that needs a refresh here.
  await restore();
  await updateBadge();
  return null;
}
...
async function showNotification(insight) {
  ...
  try {
    await chrome.notifications.create(`tb-${insight.id}`, {
      ...
      title: (insight.title || 'MarketerTwin').slice(0, 120),
...
async function handleMessage(message, sender) {
  ...
  switch (message.type) {
    ...
    default:
      if (String(message.type || '').startsWith('mt-')) return await handleMt(message);
...
const MENU_SPECS = [
  { id: 'tb-ask', title: 'Ask MarketerTwin about this page', contexts: ['page', 'action'] },
  ...
  { id: 'tb-open', title: 'Open MarketerTwin Studio', contexts: ['action'] }
];
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  switch (info.menuItemId) {
    case 'tb-ask': {
      ...
      await chrome.tabs.create({ url: chrome.runtime.getURL(`app/app.html?ask=${encodeURIComponent(q)}`) });
      break;
    }
    case 'tb-search-selection': {
      const text = String(info.selectionText || '').trim().slice(0, 200);
      if (text) await chrome.tabs.create({ url: chrome.runtime.getURL(`app/app.html?ask=${encodeURIComponent(text)}`) });
      break;
    }
    case 'tb-save-now':
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  if (command === 'analyze-page') {
    await handleMessage({ type: 'tb-capture-now' }, {});
    return;
  }
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  else if (alarm.name === DAILY_ALARM) await brain.selfLearn(await getSettings(true));
  else await onMtAlarm(alarm.name);
});
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (details.reason === 'install') {
    await chrome.tabs.create({ url: chrome.runtime.getURL('app/app.html?welcome=1') });
  }
});
