/**
 * The on-device brain: your marketing twin's brain lives in the extension now.
 *
...
import { salesAnswer, salesStats } from './sales.js';
import { learnFromText, stylePromptBlock } from './style.js';
...
export async function answer(query, options = {}, settings = {}, hooks = {}) {
  ...
  void learnFromText(query, 'chat'); // style learning: never blocks, never throws
  ...
  // --- SALES PLAYBOOK: the tactic, the psychology, bilingual scripts --------
  if (parsed.type === 'sales') {
    const salesOut = salesAnswer(parsed.raw, { name, urdu: isRomanUrdu(parsed.raw).isUrdu });
    if (salesOut) {
      await store.putConversation({ query: parsed.raw, mode: 'sales', type: 'sales',
                                    topic: salesOut.tactics.join(' '), answer: truncate(salesOut.text, 300) });
      return {
        mode: 'chat', text: salesOut.text, grounded: true, citations: salesOut.citations,
        followups: ['What comes after this stage?', 'Give me the Roman Urdu version',
                    'How do I follow up if they still say no?'],
        sales: { tactics: salesOut.tactics, stage: salesOut.stage },
        conversation: { name, empathy: empathyLine(parsed.raw), emotion: emotion.primary,
                        tone: tone.mood },
      };
    }
    // no honest playbook match -> fall through to core/fluent machinery
  }
  ...
  if (parsed.type === 'books' || parsed.type === 'success') {
    ...
    return {
      ...
      followups: ['Tell me about Influence', 'Tell me about DotCom Secrets',
  ...
  if (!finalResult.grounded && !usedWeb && !memoryOnly && !storyMode &&
      (parsed.type === 'knowledge' || parsed.type === 'compare' || parsed.type === 'define' || parsed.type === 'sales' || parsed.type === 'advice')) {
  ...
  if (!finalResult.grounded && !usedWeb && !memoryOnly && !storyMode && !explicitWeb &&
      (parsed.type === 'knowledge' || parsed.type === 'sales')) {
  ...
  if (provider) {
    ...
    const messages = await buildNeuralMessages({
...
/**
 * THE PRE-MADE SYSTEM PROMPT — the fully-acted sales-master persona that
 * drives any connected LLM "our extension way". Users can replace it in
 * Settings > AI Provider > Custom system prompt; this is the default act.
 */
function SALES_MASTER_PROMPT(owner) {
  return [
    `You are MarketerTwin — ${owner} personal AI twin AND a world-class digital-marketing sales master living inside their browser extension. You have the combined field experience of a 7-figure Meta Ads media buyer, a direct-response copywriter, a branding strategist and a closing-trained sales consultant — and the warmth of an honest friend.`,
    '',
    'WHO YOU ARE (stay in this act on every reply):',
    '- A digital marketer who has personally managed Meta (Facebook/Instagram) ad budgets from 500 PKR/day tests to seven-figure monthly spends: campaign structure, objectives, Advantage+, learning phase, auction dynamics, CAPI/pixel tracking, attribution windows, creative testing, scaling and killing campaigns.',
    '- A sales psychologist: Cialdini principles, objection handling (price, trust, timing, authority), discovery questioning, closing and negotiation — you always give the tactic AND the ready-to-send script.',
    '- A branding and copywriting coach: positioning, story-driven messaging, hooks, offers, funnels, value ladders.',
    '- A mentor for the Pakistani market: local businesses, COD e-commerce, WhatsApp commerce, salons/clinics/gyms/real-estate, freelancers and agencies — you know their budgets, festivals, objections and buying behaviour.',
    '',
    'HOW YOU TALK:',
    '- Mirror the user language: if they write Roman Urdu, answer in natural Roman Urdu (English terms for tech words are fine and expected); if English, answer English. Never mix awkwardly.',
    '- Friendly, talkative, encouraging — like a smart dost who has done it before. Use their name when natural.',
    '- FULLY EXPLAINED answers with real examples and numbers ("1,000 PKR/day on a 50-km radius..."), never one-liners. Teach the why behind every recommendation.',
    '- When giving scripts (DMs, ads, follow-ups), provide them word-for-word and ready to send — in the language the client speaks.',
    '- Ask ONE practical follow-up question when it helps, e.g. budget, niche or what they already tried.',
    '',
    'HONESTY RULES (never break):',
    '- Only claim knowledge of THEIR business/pages/history from the MEMORY and WEB sections below — cite them with [n]/[Wn]. Never invent memories, pages, dates or results.',
    '- If retrieval found nothing about their history, say so plainly, then answer from your general expertise.',
    '- Never promise guaranteed ad results; always give realistic ranges and the levers that move them.',
    '- No black-hat tactics: no fake scarcity, fake reviews, engagement bait, or policy-violating advice — Meta policy compliance is part of every recommendation.',
  ].join('\n');
}

async function buildNeuralMessages(input) {
  const {
  ...
  const customPrompt = String(settings.customSystemPrompt || '').trim();
  system.push(customPrompt || SALES_MASTER_PROMPT(owner));
  ...
  const styleText = await stylePromptBlock();
  if (styleText) system.push('', styleText);
  if (summary) {
...
function metaAnswer(stats) {
  const lines = [
`Here's your marketing twin, live from this browser:`,
  ...
  lines.push('• Ask me about any of it, say “summarise my day”, or ask a marketing question — sales scripts, Meta Ads, copy, funnels.');
  return lines.join('\n');
}
...
export async function buildDigest(settings = {}, { notify = true } = {}) {
  ...
  if (notify) await maybeNotify(settings, {
    title: 'MarketerTwin daily recap',
    message: text,
  });
  return text;
}
...
export async function selfLearn(settings = {}) {
  ...
  if ((settings.webPermission || 'ask') === 'always' && settings.autoEnrich !== false) {
    ...
    let used = budget.day === today() ? budget.used : 0;

    // the daily marketing curriculum: one rotating industry topic a day, so
    // the twin's knowledge grows even before the user has browsing history
    const MKT_TOPICS = [
      'Meta Ads new features and updates',
      'Facebook advertising policy changes',
      'digital marketing trends this year',
      'Instagram Reels marketing tips for business',
      'WhatsApp Business marketing strategies',
      'e-commerce Pakistan cash-on-delivery trends',
      'AI tools for digital marketers',
      'local business marketing ideas that work',
      'Facebook Ads benchmark CPM CTR by industry',
      'personal branding on LinkedIn for marketers',
    ];
    const dayIdx = Math.floor(Date.now() / 86400000) % MKT_TOPICS.length;
    const topics = [];
    if (state.interests[0]) topics.push(`latest updates about ${state.interests[0].topic}`);
    topics.push(MKT_TOPICS[dayIdx]);

    for (const topic of topics) {
      if (used >= cap) break;
      try {
        const web = await runWebSearch(topic, settings);
        if (web.results && web.results[0]) await deepReadTopResult(web.results[0], topic);
      } catch { /* a failed search must not kill the nightly pass */ }
      used += 1;
      await store.setMeta('enrichBudget', { day: today(), used });
    }
  }
...
/** Full local wipe: every memory store cleared, in-memory state reset.
 *  Settings live in chrome.storage (settings.js) and survive unless wiped there. */
export async function wipeBrain() {
  const chunks = await store.getAllChunks();
  const pageIds = [...new Set(chunks.map((chunk) => chunk.pageId))];
  for (const pageId of pageIds) await store.deleteChunksByPage(pageId);
  await store.clearCachedPages();
  await store.clearQueue();
  await store.putInterests([]);
  await store.setMeta('facts', {});
  state.chunks = [];
  state.index = new LexicalIndex();
  state.pages = new Map();
  state.interests = [];
  state.loaded = false;
  state.loading = null;
  await ensureLoaded();
  return { ok: true, wipedPages: pageIds.length };
}
